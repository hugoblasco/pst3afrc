 ...
 */
    ...
    public static void saveInFile(ArrayList<Match> listMatch)
    {
	    ...
	    ResultSet rs;
            ...
            PrintWriter writer = new PrintWriter("cote_betclic.txt", "UTF-8");
...
//		st.executeUpdate("UPDATE MATCHS SET A_VENIR = 1 WHERE ID = "+id);
		...
		rs=st.executeQuery("SELECT * FROM COTES WHERE ID_MATCH = "+id);

		if(rs.next()){

                    st.executeUpdate("UPDATE COTES SET DOM = "+m.getCoteD()+", NUL = "+m.getCoteN()+", EXT = "+m.getCoteE()+" WHERE ID_MATCH = "+id);

                }else{		    

                    st.executeUpdate("INSERT INTO COTES (ID_MATCH,DOM,NUL,EXT) VALUES ("+id+","+m.getCoteD()+","+m.getCoteN()+","+m.getCoteE()+")");
    ...
    public static Match createMatch(Date date, String section) // à donner : 1 match
    {
        ...
        {
            coteD = listOfCotation.get(2);
            coteN = listOfCotation.get(1);
            coteE = listOfCotation.get(0);
        }
        else
        {
            // Error
